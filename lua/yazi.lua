...
M.version = "13.1.2" -- x-release-please-version
...
function M.setup(opts)
  ...
  local pattern = _is_win and "*yazi/bulk-*" or "*/yazi-*/bulk-*"
      ...
      local cmd = function(cfg)
        for _, key in pairs(cfg) do
          if type(key) == "string" then
            pcall(vim.keymap.del, "t", key, { buffer = vim.g.yazi_buffer })
          end
        end
      end
