...
Since Neovim is very configurable and extensible, it's possible that some issues are caused by plugins or configuration.
This can also make it very difficult to provide a fix. To help us diagnose the issue, please follow these steps:
...
1. try to reproduce the issue
   - if you can't reproduce the issue, make changes to the file to make it more similar to your configuration until you
     can reproduce it.
1. when reporting the issue, attach the contents of your `repro.lua` file to the issue report.
...
If inspecting a complicated issue, it's recommended to "tail" the log file to see any new messages as soon as they
arrive:
...
[bat's custom tailing](https://github.com/sharkdp/bat?tab=readme-ov-file#tail--f) instructions to get colored output.
